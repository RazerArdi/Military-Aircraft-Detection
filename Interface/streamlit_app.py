...
def fetch_telemetry_data():
    ...
    if os.path.exists(ind_path):
        ...
        df_ind['Model'] = df_ind['Model'].str.strip()
        
    if df_comp is not None:
        for model_name in df_comp['Model'].unique():
            pair_name = ""
            if "Custom" in model_name: pair_name = "Confused_Pairs_Custom_CNN.csv"
            elif "EfficientNet" in model_name: pair_name = "Confused_Pairs_EfficientNetB0.csv"
            elif "MobileNet" in model_name: pair_name = "Confused_Pairs_MobileNetV2.csv"
            
            if pair_name:
                pair_path = os.path.join(REPORTS_DIR, pair_name)
                if os.path.exists(pair_path):
                    confused_data[model_name] = pd.read_csv(pair_path)
    ...
    with col_output:
        ...
        if uploaded_file:
            ...
            # --- LOGIKA UNTUK VIDEO ---
                ...
                st_data = st.empty()  # Placeholder untuk teks data
                ...
                while cap.isOpened():
                    ...
                    # 1. Preprocess (Sama seperti Image)
                    ...
                    color = (35, 134, 54) if score > 0.75 else (210, 153, 34) 
                    
                    # Kotak Statis di Tengah (Simulasi Lock)
                    center_x, center_y = w // 2, h // 2
                    box_size = int(min(h, w) * 0.3)
                    
                    # Corner Brackets
                    cv2.rectangle(frame_rgb, (center_x - box_size, center_y - box_size), 
                                             (center_x + box_size, center_y + box_size), color, 2)
                    
                    # Label
                    info_text = f"TRACKING: {label} ({score:.2f})"
                    cv2.rectangle(frame_rgb, (center_x - box_size, center_y - box_size - 30), 
                                             (center_x + box_size, center_y - box_size), color, -1)
                    cv2.putText(frame_rgb, info_text, (center_x - box_size + 5, center_y - box_size - 8), 
                                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
                    
                    # Tampilkan Frame
                    st_frame.image(frame_rgb, use_container_width=True, caption="LIVE FEED PROCESSING")
                    
                    # Tampilkan Data Teks Ringkas (Optional agar ringan)
                    st_data.markdown(f"**CURRENT LOCK:** `{label}` | **CONFIDENCE:** `{score:.2%}`")
